    ...
    <div className="app-layout">
      ...
      <aside className="sidebar">
        ...
        <div style={{ padding: '1.5rem', borderTop: '1px solid #e2e8f0' }}>
          <div className="nav-item" onClick={() => setToast({ visible: true, message: 'Settings portal opening...' })}><Settings size={20}/> Settings</div>
          <div className="nav-item" onClick={() => setToast({ visible: true, message: 'Support ticket system active.' })}><HelpCircle size={20}/> Support</div>
      ...
      <div className="main-area">
        ...
        <header className="topbar">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
            <Bell size={20} color="#64748b" onClick={() => setToast({ visible: true, message: '5 new candidate matches.' })} style={{ cursor: 'pointer' }}/>
            <HelpCircle size={20} color="#64748b" onClick={() => setToast({ visible: true, message: 'Hiring assistant online.' })} style={{ cursor: 'pointer' }}/>
            <button className="btn btn-primary" style={{ gap: '0.5rem', borderRadius: '20px' }} onClick={() => setIsChatOpen(true)}>
              <Bot size={18}/> AI Chatbot
            </button>
            <div style={{ background: '#e2e8f0', borderRadius: '50%', padding: '0.5rem', cursor: 'pointer' }} title={companyName} onClick={() => setToast({ visible: true, message: 'Company profile coming soon.' })}><User size={20} color="#00283e"/></div>
          </div>
        </header>
        ...
        <main className="content-wrapper">
          {activeSection === 'Overview' ? (
            <>
              <div className="grid-3" style={{ gridTemplateColumns: '2fr 1fr', marginBottom: '2rem' }}>
                ...
                <div className="card card-dark" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center' }}>
                   ...
                   <div style={{ display: 'flex', gap: '1rem' }}>
                     <button className="btn card-peach" style={{ border: 'none', fontWeight: 'bold' }} onClick={() => setToast({ visible: true, message: 'Opening Job Creation Wizard...' })}><PlusCircle size={18} style={{ marginRight: '8px' }}/> Job Posting</button>
                     <button className="btn" style={{ background: '#004b73', color: 'white' }} onClick={() => setActiveSection('Jobs')}><Eye size={18} style={{ marginRight: '8px' }}/> View All Jobs</button>
                   </div>
                </div>
              ...
              <div className="grid-4" style={{ marginBottom: '3rem' }}>
                {filteredApps.length > 0 ? filteredApps.map((app, idx) => (
                   <div key={app._id} className="card" style={{ background: '#f8fafc', border: 'none', cursor: 'pointer' }} onClick={() => setToast({ visible: true, message: `Viewing profile: ${app.studentId?.name}` })}>
              ...
              <div className="flex-between" style={{ marginBottom: '1.5rem' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '1rem' }}>
                  ...
                  <MoreHorizontal size={20} color="#64748b" style={{ cursor: 'pointer' }} onClick={() => setToast({ visible: true, message: 'Pipeline automation options...' })}/>
                </div>
              </div>
              ...
              <div className="grid-3" style={{ alignItems: 'flex-start' }}>
                ...
                <div>
                  ...
                  {filteredApps.filter(a => a.status === 'Applied').map(app => (
                    <div key={app._id} className="card" style={{ marginBottom: '1rem', borderLeft: '4px solid #004b73', cursor: 'pointer' }} onClick={() => setToast({ visible: true, message: 'Moving to Shortlist...' })}>
                ...
                <div>
                  ...
                  {filteredApps.filter(a => a.status === 'Shortlisted').map(app => (
                    <div key={app._id} className="card" style={{ marginBottom: '1rem', borderLeft: '4px solid #0f766e', cursor: 'pointer' }} onClick={() => setToast({ visible: true, message: 'Scheduling Interview...' })}>
                ...
                <div>
                  ...
                  {filteredApps.filter(a => a.status === 'Interview').map(app => (
                    <div key={app._id} className="card card-peach" style={{ borderLeft: '4px solid #d15112', marginBottom: '1rem', cursor: 'pointer' }} onClick={() => setToast({ visible: true, message: 'Opening Feedback panel...' })}>
                      <h4 style={{ color: '#00283e' }}>{app.studentId?.name}</h4>
            ...
            <div className="card flex-center" style={{ height: '400px', flexDirection: 'column', gap: '1rem' }}>
               ...
               <p style={{ color: '#64748b' }}>The AI is analyzing candidate telemetry for this section.</p>
               <button className="btn btn-primary" style={{ marginTop: '1rem' }} onClick={() => setActiveSection('Overview')}>Return to Overview</button>
            </div>
          )}
        </main>
      </div>
