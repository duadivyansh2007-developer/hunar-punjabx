    ...
    <div className="app-layout">
      ...
      <aside className="sidebar">
        ...
        <div style={{ padding: '1.5rem', borderTop: '1px solid #e2e8f0' }}>
          ...
          <div className="nav-item" onClick={() => setToast({ visible: true, message: 'Support ticket system coming soon.' })}><HelpCircle size={20}/> Support</div>
      ...
      <div className="main-area">
        ...
        <header className="topbar">
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '1.5rem' }}>
            <Bell size={20} color="#64748b" onClick={() => setToast({ visible: true, message: '3 pending drive approvals.' })} style={{ cursor: 'pointer' }}/>
            <HelpCircle size={20} color="#64748b" onClick={() => setToast({ visible: true, message: 'Institute Support active.' })} style={{ cursor: 'pointer' }}/>
            <button className="btn btn-primary" style={{ gap: '0.5rem', borderRadius: '20px', background: '#d15112' }} onClick={() => setIsChatOpen(true)}>
              <Bot size={18}/> AI Chatbot
            </button>
            <div style={{ background: '#e2e8f0', borderRadius: '50%', padding: '0.5rem', cursor: 'pointer' }} title={instituteName} onClick={() => setToast({ visible: true, message: 'Profile settings coming soon.' })}><User size={20} color="#00283e"/></div>
          </div>
        </header>
        ...
        <main className="content-wrapper">
          {activeSection === 'Overview' ? (
              ...
              <div className="grid-2" style={{ gridTemplateColumns: '1fr 1.5fr' }}>
                ...
                <div className="card">
                  <div className="flex-between" style={{ marginBottom: '1.5rem' }}>
                     ...
                     <Filter size={20} color="#94a3b8" style={{ cursor: 'pointer' }} onClick={() => setToast({ visible: true, message: 'Filtering by Department...' })}/>
                  </div>
                  ...
                  <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr', gap: '1rem', alignItems: 'center', marginBottom: '1rem' }}>
                     ...
                     <div style={{ display: 'flex', gap: '4px' }}>
                       {[1,2,3,4,5,6].map(i => <div key={i} style={{ width: '24px', height: '24px', background: i < 3 ? '#0f766e' : i < 5 ? '#5eead4' : '#ccfbf1', borderRadius: '4px', cursor: 'pointer' }} title="Click for breakdown" onClick={() => setToast({ visible: true, message: 'Analyzing Full Stack competency...' })}></div>)}
                     </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr', gap: '1rem', alignItems: 'center', marginBottom: '1rem' }}>
                     <span style={{ fontSize: '0.7rem', fontWeight: 'bold', color: '#64748b' }}>DATA SCI</span>
                     <div style={{ display: 'flex', gap: '4px' }}>
                       {[1,2,3,4,5,6].map(i => <div key={i} style={{ width: '24px', height: '24px', background: i < 3 ? '#cbd5e1' : i < 5 ? '#004b73' : '#e2e8f0', borderRadius: '4px', cursor: 'pointer' }} onClick={() => setToast({ visible: true, message: 'Analyzing Data Science competency...' })}></div>)}
                     </div>
                  </div>
                  <div style={{ display: 'grid', gridTemplateColumns: '100px 1fr', gap: '1rem', alignItems: 'center', marginBottom: '1rem' }}>
                     <span style={{ fontSize: '0.7rem', fontWeight: 'bold', color: '#64748b' }}>UI/UX DEV</span>
                     <div style={{ display: 'flex', gap: '4px' }}>
                       {[1,2,3,4,5,6].map(i => <div key={i} style={{ width: '24px', height: '24px', background: i < 3 ? '#d15112' : i < 5 ? '#fed7aa' : '#ffedd5', borderRadius: '4px', cursor: 'pointer' }} onClick={() => setToast({ visible: true, message: 'Analyzing UI/UX competency...' })}></div>)}
                     </div>
                  </div>
                ...
                <div className="card">
                   <div className="flex-between" style={{ marginBottom: '1.5rem' }}>
                     ...
                     <span style={{ color: '#0ea5e9', fontSize: '0.9rem', fontWeight: '500', cursor: 'pointer' }} onClick={() => setActiveSection('Jobs')}>View All →</span>
                   </div>
            ...
            <div className="card flex-center" style={{ height: '400px', flexDirection: 'column', gap: '1rem' }}>
               ...
               <p style={{ color: '#64748b' }}>The AI is analyzing institutional-grade telemetry for this section.</p>
